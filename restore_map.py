...
work = Path(r'c:\Users\justi\OneDrive\Personal Academia\12th Grade\Dr. Zhao Research')
case_file = work / 'WA_MMIP_Case_Dataset.xlsx'
juris_file = work / 'MMIP_50_State_Jurisdiction_Dataset.xlsx'
out_file = work / 'wa_mmip_map.html'
...
all_cases[['jurisdiction_entity', 'contested']] = pd.DataFrame(all_cases['jurisdiction_text'].apply(classify_jurisdiction).tolist(), index=all_cases.index)
...
for rec in records_sorted:
    year = rec['year']
    jurisdiction_progress.setdefault(year, {'State': 0, 'Federal/Tribal': 0, 'Local/Municipal': 0, 'Mixed/Unclear': 0, 'Contested/Contradicted': 0})
    jurisdiction_progress[year][rec['jurisdiction_entity']] += 1
    if rec['contested']:
        jurisdiction_progress[year]['Contested/Contradicted'] += 1
...
<body>
  ...
  <script>
    ...
    function updateJurisdictionSummary(year) {{
      ...
      const data = jurisdictionProgress[year] || {{'State':0,'Federal/Tribal':0,'Local/Municipal':0,'Mixed/Unclear':0,'Contested/Contradicted':0}};
      ...
      bars.innerHTML = entries.map(([name, value]) => {{
        ...
        const color = name === 'State' ? '#0ea5e9' : name === 'Federal/Tribal' ? '#10b981' : name === 'Local/Municipal' ? '#f59e0b' : name === 'Contested/Contradicted' ? '#ef4444' : '#8b5cf6';
