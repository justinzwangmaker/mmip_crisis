...
# Merge and normalize
frames = [missing_df, homicide_df]
all_cases = pd.concat(frames, ignore_index=True)


def classify_jurisdiction(text):
    text = str(text).lower()
    if 'contested' in text or 'contradict' in text or 'disputed' in text or 'checkerboard' in text or 'unclear' in text:
        contested = True
    else:
        contested = False
    if 'federal/tribal' in text or 'federal' in text or 'tribal' in text:
        ...
        entity = 'State'
    elif 'local' in text or 'municipal' in text:
        entity = 'Local/Municipal'
    else:
        entity = 'Mixed/Unclear'
    return entity, contested

all_cases['jurisdiction_entity'], all_cases['contested'] = zip(*all_cases['jurisdiction_text'].apply(classify_jurisdiction))

# Build base records with coordinates
records = []
for _, row in all_cases.iterrows():
    if pd.isna(row['case_date']):
        continue
    county = row['county'].replace(' County', '').strip() if row['county'] else 'Unknown'
    county_key = row['county'].strip() if row['county'] else 'Unknown'
    reservation_val = str(row['reservation']).strip()
    if reservation_val and reservation_val in reservation_centers:
    ...
    records.append({
        ...
        'jurisdiction_entity': row['jurisdiction_entity'],
...
# Jurisdiction progression by year
jurisdiction_progress = {}
for rec in records_sorted:
    year = rec['year']
    jurisdiction_progress.setdefault(year, {'State': 0, 'Federal/Tribal': 0, 'Local/Municipal': 0, 'Mixed/Unclear': 0})
    jurisdiction_progress[year][rec['jurisdiction_entity']] += 1

# Load WA jurisdiction summary from 50-state dataset
...
<body>
  <div class=\"layout\">
    <div class=\"sidebar\">
      ...
      <div class=\"card\">
        ...
        <div class=\"note\">This map combines the Washington MMIP case records with the statewide legal-jurisdiction framework so that each case can be viewed by year, case type, and claimed jurisdictional authority.</div>
      </div>
      <div class=\"card\">
        <strong>Washington legal framework</strong>
        <div class=\"note\">Framework: {wa_summary.get('framework', 'N/A')}<br/><br/>Contradictory: {wa_summary.get('contradictory', 'N/A')}<br/><br/>{wa_summary.get('notes', '')}</div>
      </div>
      ...
      <div class=\"card\">
        ...
        <div class=\"legend\">
          ...
          <div class=\"legend-item\"><span class=\"swatch\" style=\"background:#f59e0b\"></span>Mixed/Unclear</div>
          <div class=\"legend-item\"><span class=\"swatch\" style=\"background:#ef4444\"></span>Contested/Contradicted</div>
        </div>
      </div>
    </div>
    <div id=\"map\"></div>
  ...
  <script>
    const baseCases = {json.dumps(records_sorted)};
    const jurisdictionProgress = {json.dumps(jurisdiction_progress)};
    ...
    function popupHtml(record) {{
      return `
        <div style=\"line-height:1.4; min-width:220px;\">
          ...
          Date: ${{record.date}}<br/>
          County: ${{record.county || 'Unknown'}}<br/>
    ...
    function updateJurisdictionSummary(year) {{
      ...
      const data = jurisdictionProgress[year] || {{'State':0,'Federal/Tribal':0,'Local/Municipal':0,'Mixed/Unclear':0}};
      ...
      bars.innerHTML = entries.map(([name, value]) => {{
        ...
        const color = name === 'State' ? '#0ea5e9' : name === 'Federal/Tribal' ? '#10b981' : name === 'Local/Municipal' ? '#f59e0b' : '#8b5cf6';
